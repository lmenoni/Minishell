...
while IFS= read -r test_cmd || [ -n "$test_cmd" ]; do
	...
	# (cd "$TEST_DIR" && printf '%s\n' "$test_cmd" 'echo $? > exit_value.txt' | ./minishell | "$SHELL_PATH" > "actual/out" 2> "actual/err")
	# minishell_exit=$(cat "$TEST_DIR/exit_value.txt")
	# rm -f "$TEST_DIR/exit_value.txt"
